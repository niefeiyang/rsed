...
parser = argparse.ArgumentParser(description="A demo application for RSE. The program has two mandatory parameters. First is the git repository address, and second is a brief description of the target file.\nExample:\n    python main.py https://github.com/lektor/lektor.git \"The file related to package name.\"")
        ...
        if os.path.isfile(file_path):
            ...
            try:
                ...
                with open(file_path, 'rb') as hexfile:
                    ...
                    for chunk in iter(lambda: hexfile.read(4096), b""):
            ...
            #md5_digest = md5_hash.hexdigest()
            # 使用文件名+MD5 作为文档的 ID
            document_id = f"{file_path}_{md5_digest}"
            # 将文档存入 ChromaDB 数据库
